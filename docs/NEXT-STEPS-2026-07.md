...
| G5 | **Optimistic-locking review under scheduler concurrency** (WS-8 item) has no recorded outcome: no ADR, no version-column usage on task transitions, no concurrency test beyond `test_atomic_close.py` (which covers closure only). **CLOSED 2026-07-26 (ADR-0008):** the governed CAS was generalized to all tasks — `queue_task`/`execute_task` route every task through conditional state claims, the legacy read-then-write path was deleted, and accepted read-then-write remainders are recorded with reasons; proof `tests/integration/test_transition_concurrency.py` (two real sessions race the QUEUED and RUNNING claims; both tests fail on the pre-decision code). | grep `version`/locking over models + state machine | ~~Medium~~ closed |
